...
class Clients extends Component {
    ...
    return (
      <div className="wrapper">
        {Object.entries(this.state.fields).map(f => {
          return (
            <div key={f[0]} className="box headline-box">
              {f[0]}
            </div>
          );
        })}
  ...
  changeBoundary = factor => {
    ...
    if (factor === 1) {
      newPosition =
        this.state.position + jump >= clients.length
          ? this.state.position
          : this.state.position + jump;
    } else if (factor === -1) {
      newPosition =
        this.state.position + jump <= 0 ? 0 : this.state.position + jump;
    }
  ...
  filterBy = () => {
    let clients = this.state.clients.slice(
      this.state.position,
      this.state.position + this.state.jump
    );
